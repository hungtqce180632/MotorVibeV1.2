...
public class AppointmentDAO {
    ...
    public boolean checkHaveAppointment(int cus_id) {
        String sql = 
            ...
            + "FROM orders \n"
            + "WHERE customer_id = ? \n"
            + "  AND (deposit_status = 0 OR order_status = 0);";
    ...
    public boolean createAppointment(String date_start, String note, int customer_id, String date_end) throws SQLException {
        // first check if the user has unfinished business
        ...
        String sqlInsert = "INSERT INTO appointments (customer_id, employee_id, date_start, date_end, note) "
                         + "VALUES (?, ?, ?, ?, ?)";
    ...
    public boolean changeAppointment(int appointment_id) throws SQLException {
        String sql = "UPDATE appointments SET appointment_status = 1 WHERE appointment_id = ?";
